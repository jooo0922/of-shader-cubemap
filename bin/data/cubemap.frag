...
in vec3 fromCam; // 큐브메쉬의 오브젝트 공간 버텍스들을 그대로 가져와서 보간한 각 프래그먼트 위치값 -> 큐브메쉬의 원점을 중심으로 둘러싼(이라고 가정한) 큐브맵으로 향하는 방향벡터로 사용함. 
...
void main(){
  ...
  // '큐브메쉬의 오브젝트 공간 원점 -> 그걸 둘러싼 큐브맵'으로 향하는 방향벡터 삼아서 사용하는 것!
